...
```{r}
DurhamHigh<-c(51,60, 48, 47, 44, 39,37,37,36,49)
DurhamLow<- c(38,38,40,34,29,28,19,24,29,30)
...
DurhamHighC <- FtoC(DurhamHigh)
DurhamLowC <- FtoC(DurhamLow)
```
``` {r}
#From Autumn Dunn to Everyone:  01:47 PM
once per machine!
From Max Hermanson to Everyone:  01:48 PM
The videos are great
From Autumn Dunn to Everyone:  02:25 PM
DurhamHigh<-c(51,60, 48, 47, 44, 39,37,36,49)
DurhamLow<- c(38,38,40,34,29,28,19,24,29,30)
DurhamHigh<-c(51,60, 48, 47, 44, 39,37,36,49)
DurhamLow<- c(38,38,40,34,29,28,19,24,29,30)

DurhamTemp<-data.frame(DurhamHigh,DurhamLow)

x_name <- "High Temperature"
y_name <- "Low Temperature"

names(DurhamTemp) <- c(x_name, y_name)
ToCel<- function(x) {
  (x - 20) / 2
}

DurhamTempC<-ToCel(DurhamTemp)
...
```{r}
DurhamTemp <- data.frame(DurhamHigh,DurhamLow,DurhamHighC,DurhamLowC)
DurhamTemp
```
