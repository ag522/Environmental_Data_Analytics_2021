...
TS_Temp <- ts(BRA_complete$Temperature_C,
              start =c(1901,1), frequency = 12)

TS_Rain <- ts(BRA_complete$Rainfall,
              start =c(1901,1), frequency = 12)
...
TS_Temp_Component <- as.data.frame(TS_Temp_Decomposed$time.series)

TS_Rain_Component <- as.data.frame(TS_Rain_Decomposed$time.series)


# Exercise 10: Visualize how the trend maps onto the data for both series

plot3 <- ggplot(TS_Temp_Component, aes(x=date))
...
Rain_trend <- trend::smk.test(TS_Rain)
summary(Rain_trend)

Temp_trend <- trend::smk.test(TS_Temp)
summary(Temp_trend)
