...
LR1 <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(LR1)
...
```{r}
TempModel <- lm (data = EPAair, PM2.5 ~ Ozone  + SITE_LATITUDE + SITE_LONGITUDE + Month + Year + Date )
summary(TempModel)
step(TempModel)

#Im getting Year, Date, Month, Ozone
...
LR2 <- lm(data = EPAair, PM2.5 ~ Ozone + SITE_LATITUDE)
summary(LR2)
```
